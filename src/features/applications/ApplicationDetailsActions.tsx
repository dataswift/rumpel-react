  ...
  return (
    <ActionMenu showOptions={setup}>
      <Link to={`${appId}/permissions`} className="actions-menu-item">
        <i className="material-icons">phonelink_lock</i> <FormatMessage id="ds.hat.application.actions.permissions" />
      </Link>

      <button onClick={disableApp} className="actions-menu-item">
        <i className="material-icons">link_off</i> <FormatMessage id="ds.hat.application.actions.disconnect" />
      </button>
    </ActionMenu>
  );
};
