...
const Login: React.FC = () => {
    ...
    const login = async () => {
        try {
            ...
            if (res.parsedBody) {
                console.log(from);
                history.replace({pathname: from.pathname, search: `?token=${res.parsedBody.accessToken}`});

                console.log(res.parsedBody.accessToken)
            }
        } catch (e) {
    ...
    return (
        <div className={'login flex-column-wrapper'}>
            <InfoHeader />
            {errorMsg && (
                <NotificationBanner type={"error"} message={errorMsg} />
            )}
            ...
            <div className="data-rights-description">
                    Data rights protection ensures your HAT is always secure and that the rights to your data are
                    preserved. Your password will not be shared with the application.
            </div>

            <div className="user-actions">
                <button role="button" type="submit" className="btn btn-accent" onClick={() => login()}>Log in</button>
