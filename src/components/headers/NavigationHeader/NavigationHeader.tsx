...
export const NavigationHeader: React.FC<Props> = ({ toggleSideMenu }) => {
  ...
  return (
    <header className={'app-header nav-header'}>
      ...
      >
        <div>
          HATs are distributed systems and being private also means no one will know if you have a problem.
          <br />
          <br />
          If you have an issue with your HAT or this dashboard, please report it{' '}
          <a href={`mailto:contact@dataswift.io?subject=Support%20for%20${window.location.hostname}`}>here</a>
        </div>
      </Modal>
    </header>
  );
