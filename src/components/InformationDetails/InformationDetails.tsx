...
const InformationDetails: React.FC<Props> = (props) => {
  ...
  return (
    <div className={'ds-information-details'}>
      <div className={'ds-information-details-header'}>{header}</div>
      {description && (
        <div className={'ds-information-details-section'}>
          <h5>
            <FormatMessage id={'ds.hat.application.details.description.title'} />
          </h5>
          <div className={'ds-information-details-section-description'}>{description}</div>
        </div>
      )}
      ...
      {purpose && (
        <div className={'ds-information-details-section'}>
          ...
          <div className={'ds-information-details-section-description'}>{purpose}</div>
        </div>
      )}
        ...
        <div className={'ds-information-details-section'}>
          ...
          <div className={'ds-information-details-section-description'}>
            {requirements.map((requirement) => {
              return (
                <div className={'ds-information-details-section-requirements'} key={requirement.title}>
        ...
        <div className={'ds-information-details-section'}>
          ...
          <div className={'ds-information-details-section-screenshots'}>
            {screenshots.map((screenshot, index) => {
              return <img key={'ds-app-screenshot' + index} src={screenshot} alt={'Screenshot'} />;
            })}
          </div>
        </div>
      )}

      {informationListData && <InfoList title={'Information'} data={informationListData} />}
    </div>
  );
};
